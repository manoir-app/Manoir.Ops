...
public sealed class DockerCoreServiceCatalogTests
{
	...
	public void CreateDeploymentPlan_ShouldUseLatestTagAndHostPort81ByDefault()
	{
		...
		Assert.AreEqual("Path(`/`)", plan.Services[0].Labels["traefik.http.routers.platform-core-admin-ui-root.rule"]);
		Assert.AreEqual("platform-core-admin-ui-root-redirect", plan.Services[0].Labels["traefik.http.routers.platform-core-admin-ui-root.middlewares"]);
		Assert.AreEqual("/platform", plan.Services[0].Labels["traefik.http.middlewares.platform-core-admin-ui-root-redirect.redirectregex.replacement"]);
